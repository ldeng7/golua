...
local tcp = go.tcp
local tcp_close = go.tcp_close
local tcp_connect = go.tcp_connect
local tcp_getreusedtimes = go.tcp_getreusedtimes
local tcp_receive = go.tcp_receive
local tcp_send = go.tcp_send
local tcp_setkeepalive = go.tcp_setkeepalive
local tcp_settimeout = go.tcp_settimeout
local tcp_settimeouts = go.tcp_settimeouts
local null = go.null
local type = type
local pairs = pairs
local unpack = unpack
...
function _M.set_timeout(self, timeout)
    ...
    return tcp_settimeout(sock, timeout)
end


function _M.connect(self, host, port, pool)
    local sock = rawget(self, "_sock")
    if not sock then
        return nil, "not initialized"
    end
    ...
    return tcp_connect(sock, host, port, pool)
end
...
function _M.get_reused_times(self)
    ...
    return tcp_getreusedtimes(sock)
end
...
local function _read_reply(self, sock)
    local line, err = tcp_receive(sock)
    if not line then
    ...
    if prefix == 36 then    -- char '$'
        ...
        local data, err = tcp_receive(sock, size)
        if not data then
            if err == "timeout" then
                tcp_close(sock)
            end
        ...
        local dummy, err = tcp_receive(sock, 2) -- ignore CRLF
        if not dummy then
            return nil, err
        end
...
local function _do_cmd(self, ...)
    ...
    local bytes, err = tcp_send(sock, req)
    if not bytes then
        return nil, err
    end
...
function _M.commit_pipeline(self)
    ...
    local bytes, err = tcp_send(sock, reqs)
    if not bytes then
        return nil, err
    end
