...
local function _recv_packet(self)
    ...
    local data, err = tcp_receive(sock, 4) -- packet header
    if not data then
...
function _M.connect(self, opts)
    ...
    ok, err = tcp_connect(sock, host, port, pool)
    if not ok then
        return nil, 'failed to connect: ' .. err
    end

    local reused = tcp_getreusedtimes(sock)
    ...
    if use_ssl then
        ...
        local ok, err = tcp_sslhandshake(sock, false, nil, ssl_verify)
        if not ok then
